...
def save_images_and_masks(images, masks, types, images_dir, masks_dir, tissues_dir, Neoplastic_dir, inflams_dir, Connective_dir, Dead_dir, Epithelial_dir, fold):
    ...
    for i in range(masks.shape[0]):
        # Define file paths
        image_path = os.path.join(images_dir, f'PaNuke_fold{fold}_{types[i]}_image_{i}.npy')
        mask_path = os.path.join(masks_dir, f'PaNuke_fold{fold}_{types[i]}_mask_{i}.npy')
        tissue_path = os.path.join(tissues_dir, f'PaNuke_fold{fold}_{types[i]}_tissueType_{i}.npy')

        Neoplastic_path = os.path.join(Neoplastic_dir, f'PaNuke_fold{fold}_{types[i]}_Neoplastic_{i}.npy')
        inflam_path = os.path.join(inflams_dir, f'PaNuke_fold{fold}_{types[i]}_Inflam_{i}.npy')
        Connective_path = os.path.join(Connective_dir, f'PaNuke_fold{fold}_{types[i]}_Connective_{i}.npy')
        Dead_path = os.path.join(Dead_dir, f'PaNuke_fold{fold}_{types[i]}_Dead_{i}.npy')
        Epithelial_path = os.path.join(Epithelial_dir, f'PaNuke_fold{fold}_{types[i]}_Epithelial_{i}.npy')
...
if __name__ == "__main__":
    for fold in [1, 2, 3]:
        images_dir = f"./PaNuke/preped/fold{fold}/images"
        masks_dir = f"./PaNuke/preped/fold{fold}/masks"
        tissues_dir = f"./PaNuke/preped/fold{fold}/tissues"
        Neoplastic_dir = f"./PaNuke/preped/fold{fold}/Neoplastic"
        inflams_dir = f"./PaNuke/preped/fold{fold}/inflams"
        Connective_dir = f"./PaNuke/preped/fold{fold}/Connective"
        Dead_dir = f"./PaNuke/preped/fold{fold}/Dead"
        Epithelial_dir = f"./PaNuke/preped/fold{fold}/Epithelial"

        # Read the data
        masks = np.load(f"./PaNuke/raw_data/Fold {fold}/masks/fold{fold}/masks.npy")
        types = np.load(f"./PaNuke/raw_data/Fold {fold}/images/fold{fold}/types.npy")
        images = np.load(f"./PaNuke/raw_data/Fold {fold}/images/fold{fold}/images.npy")
